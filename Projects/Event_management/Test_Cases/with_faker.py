...
name = driver.find_element(*name_field_locator).send_keys(full_name)

username_field_locator = By.CSS_SELECTOR, "input[name='username']"
username = driver.find_element(*username_field_locator).send_keys(first_name)

email_field_locator = By.CSS_SELECTOR, "input[name='email']"
email = driver.find_element(*email_field_locator).send_keys(email)

password_field_locator = By.CSS_SELECTOR, "input[name='password']"
password = driver.find_element(*password_field_locator).send_keys(password)

conPwd_field_locator = By.CSS_SELECTOR, "input[name='confirm_password']"
conpwd = driver.find_element(*conPwd_field_locator).send_keys(con_password)

number_field_locator = By.CSS_SELECTOR, "input[name='mobile_no']"
number = driver.find_element(*number_field_locator).send_keys(number)
sleep(10)
